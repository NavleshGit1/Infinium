...
/**
 * Render AI Food Analysis view with image upload and results
 */
function renderFoodAnalysis() {
    ...
    container.innerHTML = `
        <h2 class="text-3xl font-bold text-slate-900 dark:text-white mb-2">AI Food Analysis</h2>
        <p class="text-slate-600 dark:text-slate-400 mb-6">Upload food images from ESP32 or your device for AI-powered analysis</p>
        
        <div class="space-y-6">
            <!-- Image Upload Section -->
            <div class="bg-white dark:bg-slate-800 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 p-8 text-center nature-card">
                <input type="file" id="food-image-input" accept="image/*" class="hidden" onchange="const file = this.files[0]; if(file) handleFoodImageUpload(file)">
                <button onclick="document.getElementById('food-image-input').click()" class="inline-block mb-4 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition">
                    📸 Select Image
                </button>
                <p class="text-slate-600 dark:text-slate-400 text-sm">or drag and drop an image</p>
                <p class="text-xs text-slate-500 dark:text-slate-500 mt-2">PNG, JPG, GIF up to 10MB</p>
                
                <!-- Drag and drop handler -->
                <script>
                const uploadZone = event.currentTarget;
                const inputElement = document.getElementById('food-image-input');
                
                uploadZone?.addEventListener('dragover', (e) => {
                    e.preventDefault();
                    uploadZone.classList.add('bg-blue-50', 'dark:bg-blue-900');
                });
                
                uploadZone?.addEventListener('dragleave', () => {
                    uploadZone.classList.remove('bg-blue-50', 'dark:bg-blue-900');
                });
            ...
            <div id="food-analysis-results">
                <div class="bg-slate-50 dark:bg-slate-700 rounded-lg p-8 text-center">
                    <p class="text-slate-600 dark:text-slate-400">No food analyzed yet</p>
                </div>
            </div>
            
            <!-- Diet Plan Generation -->
            <div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6 nature-card">
                <h3 class="text-xl font-semibold text-slate-900 dark:text-white mb-4">Generate Diet Plan</h3>
                <p class="text-slate-600 dark:text-slate-400 mb-4">Based on your recent food analyses, generate a personalized 7-day diet plan.</p>
                <button id="generate-plan-btn" onclick="generatePersonalizedDietPlan()" class="px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition">
                    🥗 Generate Diet Plan
                </button>
            </div>
            
            <!-- Diet Plan Results -->
            <div id="diet-plan-results"></div>
            
            <!-- Food Analysis History -->
            <div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6 nature-card">
                <h3 class="text-xl font-semibold text-slate-900 dark:text-white mb-4">Analysis History</h3>
                <div id="food-history-container" class="space-y-4">
        ...
        <script>
            // Initialize food analysis on view load
            initFoodAnalysis();
            renderFoodAnalysisHistory();
        </script>
    `;
}
